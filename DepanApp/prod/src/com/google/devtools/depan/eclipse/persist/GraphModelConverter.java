...
public class GraphModelConverter implements Converter {
  ...
  // TODO(leeca): move this into a GraphNodeConverter class
  protected void marshalNode(GraphNode node,
      HierarchicalStreamWriter writer, MarshallingContext context) {
    String nodeLabel = mapper.serializedClass(node.getClass());
    writer.startNode(nodeLabel);
    context.convertAnother(node);
    writer.endNode();
  }

  protected void marshalEdge(GraphEdge edge,
      HierarchicalStreamWriter writer, MarshallingContext context) {
    writer.startNode("graph-edge");
